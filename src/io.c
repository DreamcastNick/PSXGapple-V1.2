...
void IO_FindFile(CdlFILE *file, const char *path)
{
    ...
    {
        // Check if the path is for DISC1.ID, DISC2.ID, and DISC3.ID
        if (strcmp(path, "\\DISC1.ID;1") == 0 || strcmp(path, "\\DISC2.ID;1") == 0 || strcmp(path, "\\DISC3.ID;1") == 0)
        {
            printf("[IO_FindFile] %s not found, skipping...\n", path);
            return; // Skip further error handling for DISC1.ID, DISC2.ID, and DISC3.ID
        }
        
        // For other files, handle the error
