...
int CheckDiscIndicator(void) {
    ...
    if (IO_FindFile(&disc1File, "\\DISC1.ID;1") || IO_FindFile(&disc2File, "\\DISC2.ID;1") || IO_FindFile(&disc3File, "\\DISC3.ID;1")) {
        return 1; //  DISC1.ID, DISC2.ID, or DISC3.ID found
    }
...
void addFileToList(const char *filename) {
    FileNode3 *newNode3 = (FileNode3 *)malloc(sizeof(FileNode3));
    strcpy(newNode3->name, filename);
    newNode3->next = fileList3;
    fileList3 = newNode3;
}
...
void loadAllFiles(void) {
    FileNode3 *current3 = fileList3;
    while (current3 != NULL) {
        loadFile(current3->name);
