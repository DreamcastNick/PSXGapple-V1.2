...
void addFileToList(const char *filename) {
    FileNode2 *newNode2 = (FileNode2 *)malloc(sizeof(FileNode2));
    strcpy(newNode2->name, filename);
    newNode2->next = fileList2;
    fileList2 = newNode2;
}
...
void loadAllFiles(void) {
    FileNode2 *current2 = fileList2;
    while (current2 != NULL) {
        loadFile(current2->name);
